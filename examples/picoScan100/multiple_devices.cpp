...
void onNewFullFrame1(sick::PointCloud const& pointCloud)
{
  ...
  std::cout << "Received full frame from device 1 on port 2115." << '\n';
}

void onNewFullFrame2(sick::PointCloud const& pointCloud)
{
  std::lock_guard<std::mutex> lock(output_mutex);
  std::cout << "Received full frame from device 2 on port 2116." << '\n';
}
...
int main(int argc, char* argv[])
{
  ...
  sick::PicoScan100Configurator configurator1(httpClient1, sick::UserLevel::Service, "servicelevel");
  sick::PicoScan100Configurator configurator2(httpClient2, sick::UserLevel::Service, "servicelevel");
  try
  {
  ...
  sick::PicoScan100 driver1([&](std::exception const& ex) {
    printExceptionMessage(2115, ex);
  });
  sick::PicoScan100 driver2([&](std::exception const& ex) {
