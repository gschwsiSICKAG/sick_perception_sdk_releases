...
struct LFPradialDistanceRangeFilter
{
  ...
  struct Get
  {
    struct Response
    {
      ...
      explicit Response(bool bEnable, NumericRange<0, 200000, 0> ulDistMin, NumericRange<0, 200000, 200000> ulDistMax)
        : _bEnable(bEnable), _ulDistMin(std::move(ulDistMin)), _ulDistMax(std::move(ulDistMax))
      {}

      bool _bEnable;
      NumericRange<0, 200000, 0> _ulDistMin;
      NumericRange<0, 200000, 200000> _ulDistMax;
    };
 ...
 This function requires at least user level: Authorized Client.
  ...
  struct Post
  {
    struct Request
    {
      ...
      explicit Request(bool bEnable, NumericRange<0, 200000, 0> ulDistMin, NumericRange<0, 200000, 200000> ulDistMax)
        : _bEnable(bEnable), _ulDistMin(std::move(ulDistMin)), _ulDistMax(std::move(ulDistMax))
      {}
