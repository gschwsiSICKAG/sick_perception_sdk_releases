...
TEST_P(AmbientLightParserTest, valid_data_can_be_parsed)
{
  ...
  auto const parsedData           = sick::compact::ambient_light::Parser::validateAndParse(data, validateChecksum);

  EXPECT_EQ(0x02020202, parsedData.telegramHeader.startOfFrame);
  EXPECT_EQ(sick::compact::TelegramType::AmbientLight, parsedData.telegramHeader.telegramType);
...
TEST(AmbientLightParserTest, does_not_crash_with_empty_data)
{
