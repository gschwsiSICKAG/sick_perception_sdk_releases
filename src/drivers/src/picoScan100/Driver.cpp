...
// EncoderReceiver
// --------------------------------------------------------------
Driver::EncoderReceiver::EncoderReceiver(std::function<void(std::exception_ptr)> const& onError)
  : m_onError(onError)
  , m_stream(std::nullopt)
  , m_onNewData(std::nullopt)
{ }

void Driver::EncoderReceiver::setup(std::uint16_t receiverPort, std::chrono::milliseconds firstDataTimeout, std::chrono::milliseconds newDataTimeout)
{
  if (m_stream)
  {
    return;
...
void Driver::EncoderReceiver::teardown()
{
  m_stream = std::nullopt;
}

void Driver::EncoderReceiver::setOnNewDataCallback(std::function<void(compact::encoder::EncoderData)> callback)
{
...
// ImuReceiver
// --------------------------------------------------------------
Driver::ImuReceiver::ImuReceiver(std::function<void(std::exception_ptr)> const& onError)
  : m_onError(onError)
  , m_stream(std::nullopt)
  , m_onNewData(std::nullopt)
{ }

void Driver::ImuReceiver::setup(std::uint16_t receiverPort, std::chrono::milliseconds firstDataTimeout, std::chrono::milliseconds newDataTimeout)
{
  if (m_stream)
  {
    return;
...
// ScanDataReceiver
// --------------------------------------------------------------
Driver::ScanDataReceiver::ScanDataReceiver(std::function<void(std::exception_ptr)> const& onError)
  : m_onError(onError)
  , m_stream(std::nullopt)
...
void Driver::ScanDataReceiver::setup(std::uint16_t receiverPort, std::chrono::milliseconds firstDataTimeout, std::chrono::milliseconds newDataTimeout)
{
  if (m_stream)
  {
    return;
...
void Driver::ScanDataReceiver::setOnNewSegmentCallback(ScanDataCallback callback)
{
  m_onNewSegmentScanData = std::move(callback);
}

void Driver::ScanDataReceiver::setOnNewSegmentCallback(PointCloudCallback callback, PointCloudConfiguration const& pointCloudConfiguration)
{
