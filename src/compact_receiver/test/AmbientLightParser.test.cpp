...
class AmbientLightParserTest : public testing::TestWithParam<sick::test::TestParams>
{ };

TEST_P(AmbientLightParserTest, valid_data_can_be_parsed)
{
  auto const p    = GetParam();
  auto const data = sick::test::readBinary(p.fileIdentifier);

  constexpr bool validateChecksum = true;
  auto const parsedData           = sick::compact::ambient_light::Parser::validateAndParse(data.begin(), data.end(), validateChecksum);

  EXPECT_EQ(0x02020202, parsedData.header.startOfFrame);
  EXPECT_EQ(sick::compact::TelegramType::AmbientLight, parsedData.header.telegramType);
  EXPECT_EQ(1, parsedData.header.telegramVersion);
}

TEST_P(AmbientLightParserTest, invalid_data_is_detected)
{
  auto const p = GetParam();
  ...
  constexpr bool validateChecksum = true;
  EXPECT_THROW(sick::compact::ambient_light::Parser::validateAndParse(data.begin(), data.end(), validateChecksum), std::exception);
}
...
TEST(AmbientLightParserTest, does_not_crash_with_reversed_iterators)
{
  auto const data = sick::test::readBinary("data/multiScan270_ambient_light-frame_0.bin");

  constexpr bool validateChecksum = true;
  EXPECT_THROW(sick::compact::ambient_light::Parser::validateAndParse(data.end(), data.begin(), validateChecksum), std::exception);
}

TEST(AmbientLightParserTest, does_not_crash_with_same_iterators)
{
  auto const data = sick::test::readBinary("data/multiScan270_ambient_light-frame_0.bin");

  constexpr bool validateChecksum = true;
  EXPECT_THROW(sick::compact::ambient_light::Parser::validateAndParse(data.begin(), data.begin(), validateChecksum), std::exception);
}
