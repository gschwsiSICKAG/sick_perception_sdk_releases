...
This example [LRS4000/configuration.cpp](LRS4000/configuration.cpp)
...
- Reads and prints the following information from the connected device: device type, firmware version, location name, order number, serial number, and device IP address
...
This example [LRS4000/data_streaming.cpp](LRS4000/data_streaming.cpp)

- Establishes a connection to the device using the specified IP address (CMD interface) or the default IP address (192.168.0.1 if no IP is specified)
- Sets the measurement data format to Compact
- Receives and prints metadata for full frame point cloud data
- Expected device configuration: Factory defaults, licenses for used functions available

## Data streaming ROI

This example [LRS4000/data_streaming_roi.cpp](LRS4000/data_streaming_roi.cpp)
...
This example [LRS4000/low_level_streaming.cpp](LRS4000/low_level_streaming.cpp)
...
This example [LRS4000/diagnosis.cpp](LRS4000/diagnosis.cpp)

- Establishes a connection to the device using the specified IP address (CMD interface) or the default IP address (192.168.0.1 if no IP is specified)
- Reads and prints device information: device type, device state, firmware version, location name, order number, and serial number
...
This example [LRS4000/write_to_pcd_file.cpp](LRS4000/write_to_pcd_file.cpp)
...
This example [LRS4000/object_detection.cpp](LRS4000/object_detection.cpp)

- Establishes a connection to the device using the specified IP address (CMD interface) or the default IP address (192.168.0.1 if no IP is specified)
- Reads and prints the following information from the connected device: product name, part number, serial number, firmware version, and device IP address
- Reads and prints the field points of the first field
- Scales all x-y coordinates by a factor of 0.5
- Reads the state of the field
- Expected device configuration: At least one field stored in object detection, object detection license available

## Change IP address

This example [LRS4000/change_ip_address.cpp](LRS4000/change_ip_address.cpp)
...
- **Linux:** `./examples/LRS4000/LRS4000_<example name>_example`
- **Windows:** `"C:\examples\LRS4000\LRS4000_<example name>_example.exe"`
