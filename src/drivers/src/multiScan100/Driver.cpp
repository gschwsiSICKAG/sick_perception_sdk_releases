...
void Driver::ScanDataReceiver::multiplexScanData(compact::scan_data::ScanData const& scanData)
{
  ...
  if (m_framePointCloud)
  {
    ...
    {
      LOG_WARNING("multiScan100") << "Received segment with no modules. Cannot deduce the frame number.";
      return;
    }
    ...
    if (!m_firstFrameChangeDetected && frameChanged)
    {
      ...
      LOG_INFO("multiScan100") << "First frame change detected. Starting to collect segments.";
      collectScanData();
      return;
    }
...
void Driver::ScanDataReceiver::setup(std::uint16_t receiverPort, std::chrono::milliseconds firstDataTimeout, std::chrono::milliseconds newDataTimeout)
{
  ...
  m_stream.emplace(
    ...
    firstDataTimeout,
    newDataTimeout,
    "multiScan100 scan data on port " + std::to_string(receiverPort)
  );
}
