...
public:
  ...
  public:
    ...
    void setup(
      ...
      std::chrono::milliseconds firstDataTimeout = MultiScan100::kDefaultFirstDataTimeout,
      std::chrono::milliseconds newDataTimeout   = MultiScan100::kDefaultNewDataTimeout
    );
    void teardown();
    void setOnNewDataCallback(DataCallback callback);

  private:
    friend class MultiScan100;
  ...
  explicit MultiScan100(std::function<void(std::exception_ptr)> const& onError = [](std::exception_ptr) -> void {});

  MultiScan100(MultiScan100 const&)                    = delete;
  auto operator=(MultiScan100 const&) -> MultiScan100& = delete;
  MultiScan100(MultiScan100&&)                         = delete;
  auto operator=(MultiScan100&&) -> MultiScan100&      = delete;
